...
ブランチ: `feature/glossary-popup`（36 commits）
バージョン: 1.17.0
...
自動テストは 441 件通過していますが、**ブラウザでの動作は一度も確認していません。** サブエージェントには Chrome を操作できず、E2E スイートはユーザーの実プロファイルを使い有料 API を消費するため実行していません。
