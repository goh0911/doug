...
## 追記（2026-08-05）: recentPairs の滞留

「新しいページの語に下線が付かない」の原因。遅延ではなく**順番が回ってこない**状態だった。

```
1 翻訳で末尾に +10（PAIRS_PER_TRANSLATION）
1 抽出で先頭から −10（EXTRACTION_PAIRS_PER_RUN、FIFO）
```

出入りが釣り合うため滞留が解消しない。実機で `recentPairs` は 33 件まで膨らんでいた。
消費は古い側からなので、いま読んでいるページのペアは常に 20 件以上の後ろに並び、
`RECENT_PAIRS_MAX`（50）に達すると**一度も抽出されないまま捨てられる**。

`EXTRACTION_PAIRS_PER_RUN = 10` の根拠は「20 ペアで台詞を丸写しする」という実測
だったが、**その破綻の真因はペア数ではなく訳ゆれ検出の出力スキーマだった**。
プロンプトを直したあと測り直した結果（2026-08-05・実機・同一 recentPairs の新しい側から）:

| ペア数 | 時間 | 丸写し | 新規 |
|---|---|---|---|
| 10 | 14.5s | 0 | 0 |
| **20** | **2.5s** | **0** | **3**（MCGEE / JOE / BETTS） |
| 33 | 2.3s | 0 | 1（CALIFORNIA） |

増やすほど良いわけではなく 20 前後が頭打ち。**20 に変更**（毎回 10 件ずつ減って捌ける）。

FIFO は維持した。新しい側から取ると、読むのが速いときに古いペアが一度も評価されず
捨てられるため。滞留が捌けるようになったので新しいページには 1〜2 回の翻訳で順番が回る。

### その場で下線を付ける（実装済み）

抽出は翻訳完了後に走り、解説の取得はさらにその次の翻訳時だったため、いま開いて
いるページの新出語には下線が付かなかった。抽出で新語が採れたらタブへ
`GLOSSARY_UPDATED` を送り、content.js が解説を取り直して
`applyGlossToRenderedOverlays()` で貼り直すようにした。

再描画の部品は既にあった（`applyGlossToRenderedOverlays` は textContent を読み直して
再分割するため冪等）。足したのは通知経路だけ。

- `runExtractionBg` が採れた語数を返す（従来は戻り値なし）
- `RECORD_SERIES_TRANSLATION` ハンドラが `added > 0` のときだけ `sender.tab.id` へ送る
- content.js の `onMessage` が `triggerGlossLoad` を呼び直す

下線は翻訳完了の数秒後（抽出 2.5 秒 + 解説取得）に後から現れる。課金は増えない——
次の翻訳で取得していたものを前倒しするだけで、24h キャッシュが重複を防ぐ。

### 下線が出るまでの時間（2026-08-05 実測）

`create` は実質ゼロで、セッションの使い回しは効かない。コストは推論そのもの。

| | |
|---|---|
| `LanguageModel.create()` | 1〜6ms |
| 1 回目の `prompt`（コールド） | 18,275ms |
| 2 回目以降の `prompt` | 約 660ms |
| 解説 1 語ぶん（取得 + 要約） | 1,904ms |

13 語 ÷ `GLOSS_CONCURRENCY`(3) × 1.9 秒 ≒ 8〜9 秒。総時間はこれ以上縮みにくいため、
**できた語から順に下線を出す**方針にした（`GLOSS_DEFS_PARTIAL`）。
最初の下線は約 2 秒で出る。

---

## 追記（2026-08-05）: Comic Vine のレート制限と、待ち時間の真因
...
| 逐次 6 回 | 5,798ms |
| 並列 6 回 | 5,846ms（同じ） |
| Wikipedia 3 件（並列） | 342ms |

**同時実行数を上げても効かない。通信も無視できる。律速は Nano の推論だけ**で
1 語あたり約 1.9 秒。したがって「生成する語数」がそのまま待ち時間になる。

にもかかわらず `loadGlossDefs` は用語集の全語（実機で 75 語）を要求していた。
描画済みオーバーレイの訳文に実在する語だけに絞った（`e163271`）。

### Comic Vine が HTTP 420 で全滅していた

実機ログで 30 語すべてが `[gloss] ComicVine HTTP 420`。420 は非標準の
"Enhance Your Calm"（レート超過）。Comic Vine は「1 秒 1 リクエスト」「リソース
あたり 1 時間 200 件」を求めているが、同時実行 3 で無間隔に叩いていた。

`isTransientHttpStatus` は 408 / 429 / 5xx しか見ておらず、**420 は恒久的失敗として
24 時間の失敗キャッシュに焼き付いていた**。混雑しただけの語が丸一日引き直されない。

- 420 を一時的失敗に追加
- Comic Vine 専用の最小間隔（1.1 秒）とクールダウン（60 秒）。
  `glossFetchCooldownUntil` を触ると制限に掛かっていない Wikipedia まで止まる
- **先読みは Wikipedia だけにした**。全語ぶんの Comic Vine 照会は 1 時間 200 件の
  上限を食い潰す。先読みで失敗した語の指紋は `3:en-wikipedia` になり、対話経路の
  `3:en-wikipedia+comicvine` と異なるため自動的に引き直される（指紋機構がそのまま効く）
- `GLOSS_PIPELINE_EPOCH` を 3 に上げ、焼き付いた失敗を失効させた

### 先読みの役割を「モデルを温める」に変えた

実機ログ `[gloss] missing terms capped: 50 -> 30`。先読みが 30 語ぶんの解説生成を
直列で抱え、`resolveGlossDefs` がシリーズ単位でロックをチェーンする（`glossInFlight`）
ため、翻訳後の要求が約 57 秒（30 × 1.9 秒）待たされていた。

測定で分かった 2 つのコストのうち、先読みが本当に前倒しできるのは
**モデルのコールドスタート**のほう:

| | |
|---|---|
| 1 回目の `prompt` | 18,275ms |
| 2 回目以降 | 約 660ms |

先読みは短いプロンプトを 1 回投げてモデルをロードさせるだけにした（`warmUpNano`）。
語ごとの解説は `loadGlossDefs` が「いまページに出ている語」だけを翻訳直後に作る。

`resolveGlossDefs` の `nanoOnly` 経路（＋そこから使う `primaryOnly`）は呼び出し元が
無くなったが、設計書 §4.1 の「先読みでは有料 API へフォールバックしない」を担保する
仕組みとして残してある。先読みで語の生成を再開する場合はここを使う。

### 解説を用語抽出より優先する

Nano は直列なので、用語抽出と解説生成は同じ資源を奪い合う。従来は
`RECORD_SERIES_TRANSLATION` ハンドラが抽出を起動しており、翻訳完了時に両者が
同時に走って抽出が先にロックを取っていた。抽出はコールドスタート時に 16〜19 秒
かかる（実機ログ: `用語抽出: 20ペア 19159ms` / `16076ms`）ため、その間ずっと
下線が出なかった。

`RECORD_SERIES_TRANSLATION` は抽出を起動せず `extractionDue` を返すだけにし、
content.js が **解説の取得を終えてから** `RUN_EXTRACTION` を送る。
抽出が次のページまで遅れても実害は無い（`recentPairs` は最大 50 件保持される）。

`RUN_EXTRACTION` ハンドラも新語が採れたら `GLOSSARY_UPDATED` を返すようにした
（シリーズ管理画面から呼ばれた場合は `sender.tab` が無いので送らない）。
