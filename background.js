...
// 当初 20 にしていたが、実機で 20 ペア / 35.9 秒 / 応答 2010 字 / 候補 0 件という
// 破綻を再現した（固有名詞ではなく台詞を丸ごと返す）。10 ペアは 6.4 秒で
// 正しく固有名詞を返していたため、境界の 20 ではなく 10 を採る
const EXTRACTION_PAIRS_PER_RUN = 10;
...
async function runExtractionBg(seriesId) {
  ...
  try {
    ...
    // 遅さは原因ではなく出力破綻の症状。
    // 追試（2026-07-29）: ペア20件 → 35.9秒 / 台詞まるごと。20 は入力次第で破綻する
    // 境界だったため 10 に下げた。
